  ...
  copyFiles(files) {
    ...
    if (this.dataModel) {
      ...
      dataModelFiles.forEach((source) => {
        const dest = source
          .replaceAll("\\", "/")
          .replace("src/datamodels/", "");
        fs.cpSync(
          source,
          `build/${this.packageName}/${dest}`,
    ...
    fs.rmSync(`build/${this.packageName}/src/datamodels`, {
      recursive: true,
      force: true,
    });
  }
  ...
  renameFiles() {
    glob(`build/${this.packageName}/**/*boilerplate*.*`).then((files) => {
      files.forEach((oldPath) => {
        ...
        fs.rename(
          oldPath,
          `${directory}/${file.replaceAll("boilerplate", this.packageName)}`,
          (err) => {
            if (err) throw err;
          }
        );
...
inquirer
  ...
  .prompt([
    {
      ...
      name: "packageName",
      message:
        'Enter the package name of your system, such as "my-system" (alphanumeric characters and hyphens only):',
      default: "my-system",
    },
    {
      type: "input",
    ...
    },
    {
      ...
      name: "className",
      message:
        'Enter the name of your system for usage in JS classes, such as "MySystem" (alphanumeric characters only):',
      default: "MySystem",
    },
    {
      type: "input",
      name: "constantName",
      message:
        'Enter the name of your system for usage in constants, such as "MY_SYSTEM" (alphanumeric characters and underscores only):',
      default: "MY_SYSTEM",
    },
    {
      type: "confirm",
  ...
  .then((answers) => {
    ...
    console.log(
      `Success! Your system has been written to the ${generator.packageName}/ directory.`
    );
  })
  // Handle errors.
  .catch((error) => {
